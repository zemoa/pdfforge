...
| Windows delivery      | A single self-contained `PDFForge.exe`, not an installer. It extracts its technical runtime into the current user's local application directory. WebView2 is relied upon for now.                      |
| CI                    | GitHub Actions validates pull requests and publishes a GitHub Release with the Linux AppImage and Windows executable for `vX.Y.Z` tags.                                                                |
| Updates and telemetry | Updates are checked only by an explicit user action against public GitHub Releases over HTTPS. Release artifacts require cryptographic signature and checksum verification. No telemetry is collected. |
| Windows signing       | Release artifacts are cryptographically signed for in-application verification. Authenticode signing remains optional in CI when a PFX certificate and password secrets are supplied.                  |
...
readable PDF without password prompting, while its PDFium-backed page renderer
emits only the current rasterized page and normalized word bounds. The renderer
...
ESLint, Prettier, strict TypeScript, Rust formatting and Clippy are mandatory. CI pins Rust 1.98.1 so its outputs are reproducible; its Cargo cache is keyed by the installed compiler fingerprint and `Cargo.lock`. On an exact-key miss, it restores the latest cache for the same operating system and compiler, then lets Cargo verify and rebuild only incompatible artifacts. No cache is shared across operating systems or Rust compiler releases. Tests are unit tests only and cover business/application behaviour in the Rust backend (and a frontend domain store only when it contains behaviour). There are no end-to-end tests in this project at this stage.
